...
async function listAllAnnouncements(scope: string[] | null): Promise<AdminRow[]> {
  let q = supabaseAdmin()
    .from("announcements")
    .select(
...
function AnnouncementRow({ r, archived }: { r: AdminRow; archived?: boolean }) {
  ...
  return (
    <Link
      href={`/portal/announcements/${r.id}`}
      style={{
        background: "#121a2e",
...
export default async function AdminAnnouncements({
  ...
  // Leads see their ministries plus church-wide (General) announcements.
  const scope = ministryScope(pu, { includeGeneral: true });
  const [rows, params] = await Promise.all([listAllAnnouncements(scope), searchParams]);
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px", flexWrap: "wrap", gap: "12px" }}>
        ...
        <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
          {pendingCount > 0 && (
            <Link
              href="/portal/announcements/pending"
              style={{
      ...
      {archived.length > 0 && !showArchived && (
        <div style={{ marginTop: "18px" }}>
          <Link href="/portal/announcements?archived=1" style={toggleLinkStyle}>
      ...
      {archived.length > 0 && showArchived && (
        <>
          ...
          >
            ...
            <Link href="/portal/announcements" style={{ ...toggleLinkStyle, padding: "8px 14px" }}>
              Hide archived
            </Link>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
