...
import { requirePortalUser, ministryScope } from "../../../lib/portal-auth";
...
export default async function AdminDashboard() {
  ...
  // Members get their own landing view; leads see their ministry's numbers;
  // pastoral sees the whole house.
  if (pu.role === "member") {
    return <MemberOverview pu={pu} greeting={greeting} />;
  }
  const scope = ministryScope(pu);

  const [counts, recent, pending] = await Promise.all([
    getDashboardCounts(scope),
    getRecentActivity(6, scope),
    listPendingEvents(scope),
  ]);

  const stats = [
    // Visitor follow-up is a pastoral surface; leads get their ministry's
    // feedback count in that slot instead.
    pu.role === "pastoral"
      ? {
          label: "I'm New",
          ...
          href: "/portal/submissions?tab=visitors",
        }
      : {
          label: "Feedback",
          value: counts.feedback,
          sub: pu.ministries.join(", ") || "your ministry",
          subColor: "#9aa3b8",
          href: "/portal/submissions?tab=feedback",
        },
    {
      ...
      href: pu.role === "pastoral" ? "/portal/events/pending" : "/portal/events",
    },
    {
      ...
      sub: "All-time",
      subColor: "#9aa3b8",
      href: "/portal/submissions?tab=prayers",
    },
    {
